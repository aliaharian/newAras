	...
	<section class="cart bgwhite p-t-70 p-b-10 " style="direction: rtl;text-align: right;">
		<div class="container">
			...
			@elseif(isset($gift_percent))
				<div class="alert alert-success" style="margin-bottom: 0!important;font-family: 'roya'">{{$gift_percent}}درصد تخفیف روی جمع خرید شما اعمال شد </div>
			@else
				@if($gift->count()==0)
				<div class="alert alert-info" style="margin-bottom: 0!important;font-family: 'roya'">کد های تخفیف خود را می توانید از بخش پنل کاربری بیابید</div>
				@endif
			@endif
				@else
				<div class="alert alert-info" style="margin-bottom: 0!important;font-family: 'roya'">برای استفاده از کد تخفیف لطفا <a href="{{route('register')}}">ثبت نام کنید</a> یا <a href="{{route('login')}}">وارد شوید</a></div>
			@endif
			<div class="flex-w flex-sb-m p-t-25 p-b-25 bo8 p-l-35 p-r-60 p-lr-15-sm">
				...
				<div class="flex-w flex-m w-full-sm">
					...
					@if($gift->count()==1)
						...
						<form method="post" action="{{route('delete-gift')}}" class="form-horizontal">
							...
							<div class="size11 m-r-10" style="float: right">
								<p>پرداختی شما: <span style="color: darkred">{{number_format($off_total_price*(100-$gift->value('percent'))/100)}}</span> تومان</p>
							</div>
					...
					<div class="size11 bo4 m-r-10" style="float: right">
						...
						<input class="sizefull s-text7 p-l-22 p-r-22" type="text" name="gift_code" placeholder="کد تخفیف">
					</div>
						<div class="size12 trans-0-4 m-t-10 m-b-10 m-r-10" style="margin: 0 10px;float: left;">
							<!-- Button -->
							<button class="flex-c-m sizefull bg1 bo-rad-23 hov1 s-text1 trans-0-4">
