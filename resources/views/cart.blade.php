                    ...
                    <table class="table-shopping-cart">
                        ...
                        @foreach($pre_order as $order)
                            <tr class="table-row">
                                ...
                                <td class="column-2">
                                    <a title="{{\App\Product::find($order->product_id)->name}}"
            ...
            @if (\Illuminate\Support\Facades\Auth::check())
                @if(isset($err))
                    <div class="alert alert-danger"
                         style="margin-bottom: 0!important;font-family: 'roya'">{{$err}}</div>
                @elseif(isset($gift_percent))
                    <div class="alert alert-success"
                         style="margin-bottom: 0!important;font-family: 'roya'">{{$gift_percent}}درصد تخفیف روی جمع خرید
                        شما
                        اعمال شد
                    </div>
                @else
                    @if($gift->count()==0)
                        <div class="alert alert-info" style="margin-bottom: 0!important;font-family: 'roya'">کد های
                            تخفیف
