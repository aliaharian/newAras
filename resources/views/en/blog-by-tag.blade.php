...
<html lang="en-us">
<head>
	<title>blog - {{str_replace('-',' ',$tag_name)}} - Aras towel </title>
	...
	<section class="bgwhite p-t-60" style="direction: ltr">
		<div class="container">
			<h1 style="font-size: 15px;padding-bottom: 10px">
				Search for tag: {{str_replace('-',' ',$tag_name)}}
			</h1>
			<div class="row">
					...
					<div class="p-l-50 p-l-0-lg">
						...
						<div class="item-blog p-b-80">
							<a hreflang="en-us" title="{{$post->english_title}}" href="{{route('en.blog.post',['blog_id'=>$post->id,'blog_title'=>str_replace(' ','-',$post->english_title)])}}" class="item-blog-img pos-relative dis-block hov-img-zoom">
								<img src="{{$post->image}}" alt="{{$post->english_title}}" title="{{$post->english_title}}">
...
{{date_format( $post->created_at,'m/d/Y ')}}
							...
							<div class="item-blog-txt p-t-33">
								<h4 class="p-b-11">
									<a hreflang="en-us" title="{{$post->english_title}}" href="blog-detail.blade.php" class="m-text24">
{{$post->english_title}}
									</a>
								</h4>
								...
								<p class="p-b-12">
									{!! substr($post->english_content, 0, 300) !!}...
								</p>

								<a hreflang="en-us" title="{{$post->english_title}}"  href="{{route('en.blog.post',['blog_id'=>$post->id,'blog_title'=>str_replace(' ','-',$post->english_title)])}}" class="s-text20">
									read more
